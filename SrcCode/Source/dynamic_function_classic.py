...
class DynamicVariables(object):
    ...
    def __init__(self, meta_js, task_id=8888):
        ...
        self.full_function_map = dict(self.basic_function_map, **self.statics_function_map_l1)
        self.num_basic = 0
        self.num_statics = 0
        ...
        for func in unique_func:
            if func in list(self.statics_function_map_l1.keys()):
                ...
                num_basic_check += 1
        if self.num_basic + self.num_statics != self.num_funcs:
    ...
    def input_analysis(self):
        ...
        nested_input = [self.meta_js['components'][i]['input'][:] for i in range(modules_len)]
        nested_length = [self.meta_js['components'][i]['data_len'][:] for i in range(modules_len)]
        input = []
        input_length = []
        ...
        if len(input) != len(input_length):
            raise ValueError('THE NUMBER OF DATA LENGTH MUST BE ALIGNED WITH THE TOTAL AMOUNT OF INPUT DATA')
    ...
    def eval_formula(self):
        ...
        self.res_basic = step_results_basic
        return self.res_statics, self.res_basic
